...
COVID_ROOT = "/data/share/project/shared_datasets/MMTEB/C-MTEB___covid_retrieval/default/0.0.0/1271c7809071a13532e05f25fb53511ffce77117"
COVID_ROOT_qerels = "/data/share/project/shared_datasets/MMTEB/C-MTEB___covid_retrieval-qrels/default/0.0.0/a9f41b7cdf24785531d12417ce0d1157ed4b39ca"
...
class CorpusGenerator:
    ...
    def _load_corpus(self, corpus_path: str, qrels_path: str):
        ...
        # 2.1 取出“相关文档”：qrels 中 score > 0 的 pid
        pos_docids = set()
        for d in qrels_ds:
            score = int(d["score"])
            if score > 0:
                pos_docids.add(d["pid"])
        print(f"[INFO] qrels 正例 docid 数量: {len(pos_docids)}")
        corpus_list = []
        min_len = 200  # 长度过滤阈值
        ...
        #    - 在 qrels 正例集合里
        #    - 长度 >= min_len
        #    - （如果配置了 score_path）且在高分 docid 集合里
        for data in tqdm(corpus, desc="Loading corpus"):
            cid = data["id"]
            text = data["text"]

            # (1) 只保留 qrels 正例
            if cid not in pos_docids:
                continue
            ...
            # ✅ 通过所有过滤，保留
            corpus_list.append({
                "_id": cid,   # 顺便把原始 id 也带上，方便 seed_id 追踪
                "text": text,
            })
