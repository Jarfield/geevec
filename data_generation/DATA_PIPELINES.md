...
3. **带焦点的多轮问题生成（复用同一份合成 doc）**：调用 `run_generation.py --task_type covidretrieval --num_rounds N`。在多轮模式下，脚本会在五类 focus 提示间循环切换——事实细节、政策措施、疫苗/治疗、风险/防护、社会影响——让同一份合成文档在不同轮次反复利用、产出多类问题。结果按轮次保存（例如 `en-triplets_round1.jsonl`），并支持跨运行去重。
...
4. **标题生成（如需三元组）**：若希望继续统一的 query 生成流程，可在完成打分清洗后调用 `run_generation.py --task_type scidocs`，生成“可能会引用该摘要的论文标题”，得到 `{query: title, pos: abstract}` 的 triplets；SCIDOCS 默认单轮，不启用叙事焦点。
