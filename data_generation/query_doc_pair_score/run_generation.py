...
def save_triplets(
    ...
    if os.path.exists(save_path):
        with open(save_path, "r", encoding="utf-8") as f:
            for line in f.readlines():
                ...
                for pos_md5 in triplet['pos']:
                    pos_md5s.add(compute_md5(pos_md5))
    ...
    with open(save_path, 'w', encoding='utf-8') as f:
        ...
        for triplet in triplets:
            ...
            # _pos_md5 = compute_md5(triplet['pos'])
...
def main(args):
    ...
    if examples_dir is not None:
        examples_path = os.path.join(f"/data/share/project/tr/mmteb/code/datasets/teccovid_generation_results/treccovid/formatted/11-27/treccovid-en-triplets.jsonl")
        try:
            with open(examples_path, 'r', encoding='utf-8') as f:
                # examples_pool = json.load(f)
                examples_pool = []
                for line in f:
                    examples_pool.append(json.loads(line))
                examples_pool = random.sample(examples_pool,
                                              min(num_samples, len(examples_pool)))   # sample 30 examples for few-shot generation
        except:
