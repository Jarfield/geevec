#!/bin/bash
source /root/miniconda3/bin/activate /root/miniconda3/envs/train_embedder

task_type="arguana"
data_dir="/share/project/psjin/data/generated_data/arguana/generation_results/prod"
corpus_dir="shared_models/datasets--mteb--arguana/snapshots/c22ab2a51041ffd869aaddef7af8d8215647e41a"

save_dir="/share/project/psjin/data/generated_data/$task_type/generation_results/hn_mine_data"
index_save_dir="/share/project/psjin/data/generated_data/$task_type/generation_results/hn_mine_index"

query_instruction_for_retrieval="Given a claim, find documents that refute the claim"

languages=("en")

for language in "${languages[@]}"; do
    echo "Processing language: $language"

    python /share/project/psjin/code/data_generation/code_for_ArguAna/hn_mine/mine_v2_modified.py \
    --embedder_name_or_path /share/project/shared_models/Qwen3-Embedding-8B \
    --embedder_model_class decoder-only-base \
    --pooling_method last_token \
    --cache_dir /share/project/shared_models \
    --query_instruction_for_retrieval "$query_instruction_for_retrieval" \
    --query_instruction_format_for_retrieval 'Instruct: {}\nQuery:{}' \
    --trust_remote_code False \
    --batch_size 128 \
    --embedder_query_max_length 512 \
    --embedder_passage_max_length 512 \
    --input_file $data_dir/$language/$task_type/$language-triplets.jsonl \
    --output_file $save_dir/$language/$task_type/$language-triplets.jsonl \
    --candidate_pool $corpus_dir/corpus.jsonl \
    --index_save_dir $index_save_dir/Qwen3-Embedding-8B/$language \
    --search_top_k 1000 \
    --negative_number 63 \
    --use_gpu_for_searching True

    echo $cmd
    eval $cmd
    echo "Finished processing language: $language"
done
