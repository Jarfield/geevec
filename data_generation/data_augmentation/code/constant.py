...
def get_generation_prompt(
    ...
    task_to_gen_instruction: Dict[TaskType, str] = {
        ...
        TaskType.ailastatutes: (
            "Given a statute in {language}, generate a situation in {language} in which the statute is perfectly applicable."
        ),
        TaskType.scidocs: (
    ...
    gen_instruction = task_to_gen_instruction[task.task_type].replace(
        "{language}", task.language.value
    )
    gen_output = task_to_gen_output[task.task_type].replace(
        "{language}", task.language.value
    )
